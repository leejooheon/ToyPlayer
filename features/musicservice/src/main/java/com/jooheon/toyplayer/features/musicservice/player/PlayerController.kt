...
class PlayerController(private val scope: CoroutineScope) {
    ...
    private val _customEvents = MutableSharedFlow<CustomCommand?>()
    val customEvents = _customEvents.asSharedFlow()

    private val internalListener = object : MediaBrowser.Listener {
        override fun onCustomCommand(
            ...
            scope.launch {
                val customCommand = CustomCommand.parse(command, args)
                _customEvents.emit(customCommand)
            }
